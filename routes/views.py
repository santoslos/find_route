...
def add_routes(request):
    if request.method == "POST":
        ...
        messages.error(request, "Невозможно сохранить несуществующий маршут")
        return redirect('/')
...
def save_route(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Маршут успешно сохранен")
            return redirect('/find_routes')
...
class RouteDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get(self, request, *args, **kwargs):
        messages.success(request, "Маршут успешно удален")
